...
declare -A PRESETS=(
  ...
  [trojan-ws]="proto=trojan|path=/|sni=youtube.com|alpn=http/1.1|memory=2048|cpu=1|instances=16|concurrency=1000|timeout=3600"
  [vless-ws]="proto=vless|path=/|sni=youtube.com|alpn=http/1.1|memory=2048|cpu=1|instances=16|concurrency=1000|timeout=3600"
  [vmess-ws]="proto=vmess|path=/|sni=youtube.com|alpn=http/1.1|memory=2048|cpu=1|instances=16|concurrency=1000|timeout=3600"
)
...
if [ "${INTERACTIVE}" = true ] && [ -z "${PRESET:-}" ] && [ $ATTEMPT_NUMBER -eq 1 ]; then
  ...
  echo -e "  ${BOLD}${BRIGHT_RED}2${NC} ${BRIGHT_RED}trojan-ws${NC}          ${DIM}TROJAN Protocol, youtube.com (Optimized)${NC}"
  echo -e "  ${BOLD}${BRIGHT_CYAN}3${NC} ${BRIGHT_CYAN}vless-ws${NC}           ${DIM}VLESS Protocol, youtube.com (Fast)${NC}"
  echo -e "  ${BOLD}${BRIGHT_YELLOW}4${NC} ${BRIGHT_YELLOW}vmess-ws${NC}           ${DIM}VMESS Protocol, youtube.com (Compatible)${NC}"
...
declare -A REGION_NAMES=(
  ...
  [us-west4]="US🇺🇸(W4)"
  [us-south1]="US🇺🇸(S1)"
  [northamerica-northeast1]="Canada🇨🇦(N1)"
  [northamerica-northeast2]="Canada🇨🇦(N2)"
  ...
  [europe-west2]="UK🇬🇧"
  [europe-west3]="Germany🇩🇪(W3)"    
  [europe-west4]="Netherlands🇳🇱" 
  [europe-west6]="Switzerland🇨🇭"
  [europe-west8]="Italy🇮🇹(W8)"
  [europe-west9]="France🇫🇷"
  [europe-west10]="Germany🇩🇪(W10)"
  [europe-west12]="Italy🇮🇹(W12)"
  [asia-east1]="Taiwan🇹🇼" 
  [asia-east2]="HK🇭🇰"
  [asia-northeast1]="Japan🇯🇵(N1)"
  [asia-northeast2]="Japan🇯🇵(N2)"
  [asia-northeast3]="S_Korea🇰🇷"
  [asia-southeast1]="Singapore🇸🇬"
  [asia-south1]="India🇮🇳"
  [australia-southeast1]="Australia🇦🇺"
  [africa-south1]="S_Africa🇿🇦"
  [me-west1]="Israel🇮🇱"
)
get_region_name() {
  local region_code=$1
